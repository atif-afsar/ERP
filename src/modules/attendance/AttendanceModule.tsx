        ...
        <div className="space-y-4">
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5">
              <div className="flex items-center gap-2 flex-1 sm:flex-initial">
                <label className="text-xs text-slate-600 font-semibold whitespace-nowrap">Target {getLabel('group')}:</label>
                <select
                  value={selectedGroupId}
                  onChange={(e) => setSelectedGroupId(e.target.value)}
                  className="w-full sm:w-auto p-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 font-semibold shadow-2xs focus:border-emerald-500 focus:bg-white"
                >
              ...
              <button
                ...
                className={`px-3 py-2 rounded-xl border text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shrink-0 ${
                  isSessionLocked
              ...
              >
                ...
                <span>{isSessionLocked ? 'Locked' : 'Session Open'}</span>
              </button>
            </div>
          ...
          <div className="block md:hidden space-y-3">
            {groupStudents.map((stu) => {
              ...
              return (
                <div key={stu.id} className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                      <img src={stu.photoUrl} alt="" className="w-10 h-10 rounded-full object-cover border border-slate-200 shrink-0" />
                      <div>
                        <p className="font-bold text-slate-900 text-sm leading-tight">{stu.firstName} {stu.lastName}</p>
                        <p className="text-[10px] text-slate-500 font-mono mt-0.5">{stu.parentPhone}</p>
                      </div>
                    </div>
                    <span className="px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200 text-[11px] font-mono font-semibold">
                      {stu.admissionNo}
                    </span>
                  </div>

                  {/* 5-Column Segmented Status Grid */}
                  <div className="grid grid-cols-5 gap-1 pt-1">
                    {([
                      { id: 'PRESENT' as AttendanceStatus, label: 'Present', activeClass: 'bg-emerald-600 text-white shadow-xs' },
                      { id: 'ABSENT' as AttendanceStatus, label: 'Absent', activeClass: 'bg-rose-600 text-white shadow-xs' },
                      { id: 'LATE' as AttendanceStatus, label: 'Late', activeClass: 'bg-amber-500 text-slate-950 font-bold shadow-xs' },
                      { id: 'HALF_DAY' as AttendanceStatus, label: 'Half', activeClass: 'bg-indigo-600 text-white shadow-xs' },
                      { id: 'EXCUSED' as AttendanceStatus, label: 'Excused', activeClass: 'bg-purple-600 text-white shadow-xs' },
                    ]).map((opt) => {
                      const isSelected = status === opt.id;
                      return (
                        <button
                          key={opt.id}
                          onClick={() => handleStatusChange(stu, opt.id)}
                          className={`py-1.5 px-0.5 rounded-lg text-[10px] font-semibold text-center transition-all cursor-pointer truncate ${
                            isSelected
                              ? opt.activeClass
                  ...
                  <div>
                    <input
                      ...
                      className="w-full p-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-emerald-500 focus:bg-white transition-colors"
                    />
                  </div>
                </div>
              );
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5 flex-nowrap">
                            {([
                              ...
                              { id: 'ABSENT' as AttendanceStatus, label: 'Absent', activeClass: 'bg-rose-600 text-white shadow-xs' },
                              { id: 'LATE' as AttendanceStatus, label: 'Late', activeClass: 'bg-amber-500 text-slate-950 font-bold shadow-xs' },
                              { id: 'HALF_DAY' as AttendanceStatus, label: 'Half Day', activeClass: 'bg-indigo-600 text-white shadow-xs' },
                              { id: 'EXCUSED' as AttendanceStatus, label: 'Excused', activeClass: 'bg-purple-600 text-white shadow-xs' },
                            ]).map((opt) => {
                              const isSelected = status === opt.id;
                              return (
